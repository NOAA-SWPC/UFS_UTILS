...
!--- define dimension
 ...
 SUBROUTINE WRITE_FV3_ATMS_NETCDF(ZS,PS,T,W,U,V,Q,VCOORD,LONB,LATB,&
                                  LEVSO,NTRACM,TRAC_NAME,NVCOORD,NTILES)

! hmhj
!       Add TRAC_NAME for tracers in flexible name io
...
!hmhj
 CHARACTER(LEN=16), INTENT(IN)      :: TRAC_NAME(NTRACM)

 CHARACTER(LEN=256)    :: TILEFILE, OUTFILE

 INTEGER               :: ID_DIM, ID_VAR, IM, JM, I
...
!hmhj
 DO I=1,NTRACM
   ERROR = NF90_DEF_VAR(NCID2,TRIM(TRAC_NAME(I)) , NF90_FLOAT, &
                             (/DIM_LON, DIM_LAT, DIM_LEV/), ID_Q(I))
   CALL NETCDF_ERR(ERROR, 'DEFINING '//TRAC_NAME(I) )
 ENDDO
...
!!hmhj
! IF( NTRACM == 5 ) Then
!   ERROR = NF90_DEF_VAR(NCID2, 'omr', NF90_FLOAT, &
!                             (/DIM_LON, DIM_LAT, DIM_LEV/), ID_OMR)
!   CALL NETCDF_ERR(ERROR, 'DEFINING OMR')
!
!   ERROR = NF90_DEF_VAR(NCID2, 'o2mr', NF90_FLOAT, &
!                             (/DIM_LON, DIM_LAT, DIM_LEV/), ID_O2MR)
!   CALL NETCDF_ERR(ERROR, 'DEFINING O2MR')
! ENDIF
...
!hmhj
 DO I=1,NTRACM
   CALL GL2ANY(0,LEVSO,Q(:,:,:,I),LONB,LATB,CUBE_3D,IM,JM,GEOLON, GEOLAT)
   DO K = 1, LEVSO
     CUBE_3D_4BYTE(:,:,LEVSO-K+1) = REAL(CUBE_3D(:,:,K),4)
   ENDDO

   ERROR = NF90_PUT_VAR(NCID2, ID_Q(I), CUBE_3D_4BYTE)
   CALL NETCDF_ERR(ERROR, 'WRITING '//TRAC_NAME(I) )
 ENDDO
...
!!hmhj
! IF ( NTRACM == 5 ) THEN
!   CALL GL2ANY(0,LEVSO,Q(:,:,:,4),LONB,LATB,CUBE_3D,IM,JM,GEOLON, GEOLAT)
!   DO K = 1, LEVSO
!     CUBE_3D_4BYTE(:,:,LEVSO-K+1) = REAL(CUBE_3D(:,:,K),4)
!   ENDDO
!   ERROR = NF90_PUT_VAR(NCID2, ID_OMR, CUBE_3D_4BYTE)
!   CALL NETCDF_ERR(ERROR, 'WRITING OMR')
!
!   CALL GL2ANY(0,LEVSO,Q(:,:,:,5),LONB,LATB,CUBE_3D,IM,JM,GEOLON, GEOLAT)
!   DO K = 1, LEVSO
!     CUBE_3D_4BYTE(:,:,LEVSO-K+1) = REAL(CUBE_3D(:,:,K),4)
!   ENDDO
!   ERROR = NF90_PUT_VAR(NCID2, ID_O2MR, CUBE_3D_4BYTE)
!   CALL NETCDF_ERR(ERROR, 'WRITING O2MR')
! ENDIF
