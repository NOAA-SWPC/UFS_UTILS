...
!
      INTEGER NSIGI,NSIL,NSIGO,                    & 
              ...
              J1,J2,JL,IJL,J,JN,JS,N,JJ1,JJ2,      &
...
!hmhj
        PRINT *,' OUTPUT TRACER NAMES : ',(TRAC_NAME(N),N=1,NTRACO) !hmhj
...
!$omp parallel do private(j)
        DO J=1,LATBO
!hmhj     CLAT(J) = SQRT(1-SLAT(J)*SLAT(J))
          CLAT(J) = PIFAC * ASIN(SLAT(J))		!hmhj
        ENDDO
        DO J=1,LATBO,2
          RLAT(J  ) =   CLAT(J/2+1)
          RLAT(J+1) = - RLAT(J)
        ENDDO
        PRINT *,' RLAT DEG ',(RLAT(J),J=1,LATBO)
...
!                            LATBO,JJ1,JJ2,SIGHEADI%IDATE,UO,VO,TO,QO)
          ENDIF
!
          IF( IDVCO  == 3 ) THEN
...
!fv3      CALL SPECSETS(SIGHEADO,SIGDATAO,IDRT)
        ...
        CALL WRITE_FV3_ATMS_NETCDF(GFSDATAO%ZS,GFSDATAO%PS,GFSDATAO%T,GFSDATAO%W,   &
                 ...
                 LONBO,LATBO,LEVSO,NTRACM,TRAC_NAME,NVCOORDO,NTILES)  
...
!hmhj
        ALLOCATE(SLAT(LATB), WLAT(LATB), RLAT(LATB))
        CALL SPLAT(IDRT,LATB,SLAT,WLAT)
!$omp parallel do private(j)
        DO J=1,LATB
          RLAT(J) = PIFAC * ASIN(SLAT(J))
        ENDDO
        PRINT *,' INPTYP=1 RLAT DEG ',(RLAT(J),J=1,LATB)
...
!hmhj
        ALLOCATE(TRAC_NAME(NTRACO))
        CALL GET_TRACERS(IDVT, NTRACO, I_CLD, I_OZN, TRAC_NAME)
        PRINT*,'OUTPUT FILE TRACERS: ',(TRAC_NAME(I),I=1,NTRACO)

!hmhj   IF (NTRACO == 3 .AND. IDVTO == 21) THEN
!hmhj     PRINT*,'OUTPUT FILE TRACERS: SPFH O3MR CLWR '
!hmhj   ELSE
!hmhj     PRINT*,'- FATAL ERROR: CHGRES ASSUMES NTRACO=3 AND IDVT=21'
!hmhj     PRINT*,'- INPUT FILE VALUES ARE ',NTRACO,IDVT
!hmhj     PRINT*,'- STOP.'
!hmhj     CALL ERREXIT(27)
!hmhj   ENDIF
...
!  GET NEW SIGMA LEVELS
        ...
        ELSE IF (NTRACO == 5 ) THEN
          PRINT *,'  RI AND CPI FROM NAMELIST '
        ELSE 
        ...
        ENDIF
!hmhj
        PRINT *,' USE  RI ',( RI(i),i=0,NTRACO)
        PRINT *,' USE CPI ',(CPI(i),i=0,NTRACO)
...
! idea add init condition for temp tracer4-5 ( o o2)
            IF (IDVT == 200) then
              CALL VINTG_IDEA(LONB,LATCH,LEVSO,NTRACO,PO,RLAT,            &
                            LATB,J1,J2,GFSHEADI%IDATE,                    &
                            GFSDATAO%U(:,J1:J2,:),GFSDATAO%V(:,J1:J2,:),   &
                            GFSDATAO%T(:,J1:J2,:),GFSDATAO%Q(:,J1:J2,:,:))
!hmhj
!             print *,' AFTER VINTG_IDEA J1 J2 RLAT ',                    &
!                                        J1,J2,(RLAT(j),j=j1,j2)
            ENDIF
        ...
        CALL WRITE_FV3_ATMS_NETCDF(GFSDATAO%ZS,GFSDATAO%PS,GFSDATAO%T,GFSDATAO%W,  &
                 GFSDATAO%U,GFSDATAO%V,GFSDATAO%Q,VCOORDO,         &
                 LONB,LATB,LEVSO,NTRACO,TRAC_NAME,NVCOORDO,NTILES)
...
!  CODE WILL NOT INTERPOLATE THEM.
     ...
     &                       SRFLAGI, TPRCPI)
      ...
      CALL READ_FV3_GRID_DATA_NETCDF('geolon',TILE_NUM,IMO,JMO,GEOLON)
      ALLOCATE(GEOLAT(IMO,JMO))
      CALL READ_FV3_GRID_DATA_NETCDF('geolat',TILE_NUM,IMO,JMO,GEOLAT)
...
!  BY THE NEW SURFACE CHGRES LOGIC.   
      ...
      CALL READ_FV3_GRID_DATA_NETCDF('orog_raw',TILE_NUM,IMO,JMO,OROGO)
      ALLOCATE(SFCOUTPUT%OROG(IJMO))
      SFCOUTPUT%OROG = RESHAPE(OROGO, (/IJMO/))
 
!     Does fv3 create unfiltered orography?  For now set to the
!     filtered orography.
