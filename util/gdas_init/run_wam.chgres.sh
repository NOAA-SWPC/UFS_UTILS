...
if [ ${MEMBER} == 'wdas' ] || [ ${MEMBER} == 'wfs' ] ; then
  CTAR=${CRES_HIRES}
#---------------------------------------------------------------------------
...
#---------------------------------------------------------------------------
  ...
  ATMFILE="${MEMBER}.t${hh}z.atmanl"
  SFCFILE="${MEMBER}.t${hh}z.sfcanl"
else  
...
&config
 ...
 tracers="sphum","spo3","liq_wat","spo","spo2"
 tracers_input="spfh","o3mr","clwmr","omr","o2mr"
/
EOF
...
if [ ${MEMBER} == 'gdas' ] || [ ${MEMBER} == 'gfs' ]; then
  ...
  SAVEDIR=$OUTDIR/${yy}${mm}${dd}/${hh}/${MEMBER}/INPUT
  copy_data
  touch $SAVEDIR/../t${hh}z.loginc.txt
fi
